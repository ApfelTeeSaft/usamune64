...
void save_file_collect_star_or_key(s16 coinScore, s16 starIndex) {
    // USAMUNE: Update star grab timer when collecting stars
    if (gUsamuneState.config.showStarGrabTimer) {
        gUsamuneState.timers.starGrabTimer = 0; // Reset star grab timer
    }
    
    // USAMUNE: Increment attempt counter for practice
    if (starIndex >= 0) { // Only for stars, not keys
        usamune_timers_increment_attempts();
    }
